...
\begin{document}
\begin{center}
\textbf{Degree Theory Blog}
\end{center}
\section{Introduction}
In this post we will concern ourselves with degrees of continuous functions over bounded, "nice" manifolds. In particular we will narrow our focus at the moment to smooth functions over bounded regions in $\mathbb{R}^n$. With that in mind let us define the following: 
...
\end{align}
\end{subequations}
In other words $y$ is in the range of $f$ over the closure of $\Omega$ but not in the image of $f$ over the boundary of $\Omega$, and if $x\in\Omega$ s.t. $f(x)=y$ then $x$ is not a critical point of $f$.\\
...
Topologically speaking the degree of $f$ between $\Omega$ and $f\left(\Omega\right)$ is the number of times $f$, in some sense, "wraps" $\Omega$ around the region $f\left(\Omega\right)$. Of course the way in which you "wrap" around the region matters so our definition will have to take this into account (think clockwise or counterclockwise around a circle). \\
...
Since $\alpha$ is not a critical point, $f$ is smooth and $\Omega$ is open we have by the Inverse Function Theorem that there exists a neighborhood, $N_{\alpha}$, of $\alpha$ lying in $\Omega$ s.t. $f$ is invertible over $f(N_{\alpha})$. However, since $f(\alpha)=y$ this implies that there does not exist $x\in N_{\alpha}\setminus\{\alpha\}$ s.t. $f(x)=y$ or put another way $N_{\alpha}\bigcap f^{-1}(y)=\alpha$. This is of course a contradiction as $\alpha$ is a limit point of $f^{-1}(y)$ (i.e. every neighborhood of $\alpha$ contains infinitely points of $f^{-1}(y)$). The proposition now follows.
\end{proof}
\end{prop}
...
Note that by the definition of the degree we can conclude readily that a solution to $f(x)=y \ \text{ for } x\in\Omega$ exists if $deg\left(f,\Omega,y\right)\neq 0$. Let's illustrate this with a simple example. Consider the following simple graph of a what we will assume to be a smooth function $f:\mathbb{R}\rightarrow\mathbb{R}$. 
\begin{center}
