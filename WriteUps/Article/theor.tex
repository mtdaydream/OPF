...
\medskip
\add{Let  $\ell_{\vu^*}$ represent an arbitrary line passing through $\vu^*$ and $\vl_{\min},\vl_{\max}$ be two points of intersection between $\partial\Omega_u$ and $\ell_{\vu^*}$. Define a homotopy $H_{\ell_{\vu^*}} : [0,1]\times\bar{\Omega}\rightarrow\R^n$ as 
	\begin{align}
		H_{\ell_{\vu^*}}(t,\vx) = F(\vx)-\left[(1-t)\vl_{\min}+t\vl_{\max}\right]\,. \label{eq:Homo}
	\end{align}
Based on the homotopy, we give the verification of robust solvability problem as \cref{lem:NaScondition}.
}
...
%Let  $\ell_{\vu^*}$ represent an arbitrary line passing through $\vu^*$. 
If $d(\Omega,H_\ell\left(\frac{1}{2},\vx\right),\vzero)\neq 0$ for each choice of $\ell = \ell_{\vu^*}$, then the system is robust solvable if and only if the following statement, i.e., the hypothesis of property (\ref{eq:Deg1}).
\begin{align}
