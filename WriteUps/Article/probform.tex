...
\txbl{There are many practical uncertainty sets developed in the previous study, such as the ellipsoidal uncertainty\cite{ben1999robust}, cardinally constrained uncertainty\cite{bertsimas2004robust}, norm uncertainty\cite{bertsimas2004price}, and some other types of uncertainty set constructed based on probability theory \cite{bandi2012tractable} \cite{bandi2014optimal} and data driven\cite{lotfi2022data}. For convenience, the definition for robustness margin is left here in its most general form so as to capture all scenarios under which a researcher may find themselves.  Depends on the choice of $r$, we treat the uncertainty as a $\ell_{\infty}$-ball to give the guarantee of the bounds for the biggest ball such that even when the actual uncertainty set are something more complex, it can still be formulated as long as it's inside of the infinity ball. } For instance it very well may be the case that only some of the dimensions of $\vu$ will have margins of uncertainty. Furthermore, one may have need of computing the robustness margin for only a subset of the dimensions of $\vu$ which pertain to problem areas or nodes of particular interest to the research.This manual restriction will of course produce a robustness margin greater than or equal to that obtained by considering all dimensions, which certainly remains an option under the current setting.
