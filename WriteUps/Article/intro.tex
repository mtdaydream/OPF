...
\medskip
More concretely, we study a system of quadratic equations $F(\bold x)=\bold u$ where $F: \mathbb{R}^n \mapsto \mathbb{R}^n$ is quadratic in $\bold x$ for $\bold  x,\bold u \in \mathbb{R}^n$.
  We consider situations where the parameters $\bold u$ are uncertain, and we are interested in guaranteeing the existence of a solution to $F(\bold x) = \bold u$ within limits on $\bold x$ and $\bold u$.
...
\subsection{Our Contributions}
  We study systems of quadratic equations, and define a \emph{robustness margin} as a measure of the system's robust feasibility (see Definition \ref{RobustDef}).
...
%\todo{Check for any new/recent papers/preprints presenting related approaches.}
