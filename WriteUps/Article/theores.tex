...
\noindent \delete{{\bfseries ToDo for Ben:} Remove the itemize loops inside {\bfseries all} proofs.}
...
\begin{proof} 
\begin{itemize}
\item[](By contradiction) Assume $\vF(\hat{\vx})\in \vF(X)\setminus\partial \vF(X)$. 
...
\begin{proof}
\begin{itemize}
\item[] If the origin lies on the boundary of $\vF(X)$ then let $\hat{\vx}\in \partial X$ such that $f(\hat{\vx})=\textbf{0}$. 
