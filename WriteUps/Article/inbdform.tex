...
\clearpage
\delete{{\bfseries ToDo for Ben:} \\
  If we're not referencing each line within the equation system, we should not be numbering them separately. So, do not use subequations I suppose. The only place where you refer to subequations is in \eqref{eq:OPTfeasrelaxLP3}[d\& e]; may be better to explicitly talk about the subequations in that case. \\
  Highlight the titles of each model separately, in bold, e.g., {\bfseries LP Feasibility Model}.\\
  Replace occurences of $Ax_i$ with $(A\vx)_i$.\\
  Replace instances of $\backslash$ref with $\backslash$cref (wherever you are referencing equations).
}
